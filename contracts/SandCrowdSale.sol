    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 public taxOnSell; // not used now, future-proof
    ...
    function buyToken(address _beneficiary, uint256 usdtAmount) external onlyWhileOpen nonReentrant {
    ...
    function extendSale(uint256 newClosingTime) external onlyOwner {
    ...
    function changeRate(uint256 newRate) external onlyOwner onlyWhileOpen {
        require(newRate > 0, "Rate must be > 0");
        rate = newRate;
    }

    function changeInitialLockInPeriodInSeconds(uint256 newPeriod) external onlyOwner onlyWhileOpen {
        require(newPeriod > 0, "Lock period must be > 0");
        initialLockInPeriodInSeconds = newPeriod;
    }

    function changeVestingInMonths(uint256 newVesting) external onlyOwner onlyWhileOpen {
        require(newVesting > 0, "Vesting must be > 0");
        vestingMonths = newVesting;
    }

    function changeUsdtToken(IERC20Extented _newUsdt) external onlyOwner onlyWhileOpen {
    ...
    function withdrawUSDT(address _to, uint256 _amount) external onlyOwner nonReentrant {
    ...
    function setTaxRates(uint256 buyTax, uint256 sellTax) external onlyOwner {
        require(buyTax <= 10000 && sellTax <= 10000, "Too high");
        taxOnBuy = buyTax;
        taxOnSell = sellTax;
        emit TaxSettingsUpdated(buyTax, sellTax);
    }
